package etu1776.framework;

import javax.servlet.*;
import javax.servlet.http.*;
import etu1776.framework.Mapping;
import etu1776.framework.modele.*;
import java.util.*;
...
public class Utile {
    ...
    public ArrayList<String> get_AllClass(String pack) throws Exception {
        ...
        for (int i = 0; i < tables.length; i++) {
            if (tables[i].length() >= 5) {
                /// nom.class
                if (tables[i].getName().substring(tables[i].getName().length() - 6, tables[i].getName().length())
    ...
    public Mapping geMap(HashMap<String, Mapping> mappingUrls, String cle) throws Exception {
        ...
        for (String key : mappingUrls.keySet()) {
            ...
            // System.out.println(key +"|" + value.getClassName() + " | " +
            // value.getMethod());
        }
        return map;
    }
    ...
    public ModeleView get_View(String cle, HashMap<String, Mapping> mappingUrls) throws Exception {
        ...
        // valueflds[i]=String.valueOf( lstobj[i].getClass().getMethod( to_getAttribu(
        // namefield ) ).invoke(lstobj[i]) );//invoker l'attribu et le caster en String
        return mv;
    }
