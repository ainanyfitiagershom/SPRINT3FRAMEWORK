...
public class AccessAllClassByPackage {
   ...
   public Class[] getAllClassByPackage(String packageAbsolute, String packageracine) throws Exception {
      ...
      if (folder.exists() == false) {
         throw new Exception("package : \"" + packageAbsolute + "\" n'existe pas ou introuvable");
      }
      ...
      for (int i = 0; i < files.length; i++) {
         ...
         if (nfile.length() >= 6) {
            ...
            if (files[i].isFile() == true
                  && nfile.substring(nfile.length() - 6, nfile.length()).compareTo(".class") == 0) {
      ...
      for (int i = 0; i < vclasses.size(); i++) {
         classes[i] = (Class) vclasses.elementAt(i);
      }
      return classes;
   }

   public void path() {
      String path = "D:\\ITU\\L2\\S4\\Sprint\\Sprint_Dossier\\etu1776\\model";
      String racine = "etu1766\\model";
   ...
   public Class[] getAllClassinAllpackageBypackageRacine(String packageAbsolute, String packageRacine)
         throws Exception {
      ...
      // D:/dodo/proj/model | model ------> path2="D:/dodo/proj" | path3="model" <--
      // packageAbsolute.substring(path2.length()+1, packageAbsolute.length())
      ...
      for (int i = 0; i < files.length; i++) {
         ...
         if (files[i].isDirectory()) { // si c'est un repertoire ... packe1
            ...
            if (files0 != null) {
               for (int j = 0; j < files0.length; j++) {
                  if (files0[j].isDirectory()) {
                     vfile.add(files0[j]);
                  }
               } // ze repertoire ihany satria le fichier.class efa ho alaina etsy ambany
            ...
            classes = this.getAllClassByPackage(files[i].getPath(), path3); // de alaina daholo le class anatin'io
                                                                            // repertoire io.
            if (classes != null) {
               for (int j = 0; j < classes.length; j++) {
                  vclass.add(classes[j]);
         ...
         if (i == files.length - 1) {// rehefa le tapitra ny tour rehetra
            if (vfile.size() > 0) { // sady nisy ny repertoire tao ambaniny no nisy lst fichier tao amin'io
                                    // repertoire io
      ...
      for (int i = 0; i < vclass.size(); i++) {
         repclass[i] = (Class) vclass.elementAt(i);
      }
      return repclass;
      // getAnnotation Methode by nom-->omena le class misy an'le method
   }

   public Class[] getAllClassByAnnotation(Class annotationclass, String packageAbsolute, String packagePath)
         throws Exception {
   ...
   public boolean isClassAnnotedAndWithValue(Class classe, Class annotationClass, String nameMethod, String value)
         throws Exception {
      Annotation annotation = null;
      ...
      if (value.compareToIgnoreCase(valeurS) == 0) {
         return true;
      } else {
         return false;
      }
   ...
   public Method[] getAllMethodsOfObjectByAnnotationname(Object object, String annotationname) throws Exception {
      ...
      Vector vmethods = new Vector();
      for (int i = 0; i < methods.length; i++) {
         annotations = methods[i].getAnnotations(); // recuperer les annotations du methods[i] pour la comparaison si
                                                    // dessous
         if (annotations != null) {
            for (int u = 0; u < annotations.length; u++) {
               if (annotations[u].getClass().getSimpleName().compareTo(annotationname) == 0) { // comparaison des
                                                                                               // chaques annotations du
                                                                                               // method au annotation
                                                                                               // name donnee
                  vmethods.add(methods[i]);
   ...
   public Method[] getAllMethodsOfObjectByAnnotationvalue(Object object, String annotationvalue, String nameMethod)
         throws Exception {
      Method[] methods = object.getClass().getDeclaredMethods();
      Annotation[] annotations = null;
      Annotation annotation = null;
      ...
      for (int i = 0; i < methods.length; i++) {
         ...
         if (annotations != null) {
            for (int u = 0; u < annotations.length; u++) {
               ...
               if (objvalue != null) {
                  if (objvalue.toString().compareTo(annotationvalue) == 0) { // comparaison des chaques annotations du
                                                                             // method au annotation name donnee
                     vmethods.add(methods[i]);
                     u = annotations.length; // si il trouve qu'une seul , on arrete le boucle
                  }
               }
      ...
      for (int i = 0; i < m.length; i++) {
         if (nameMethod.compareTo(m[i].getName()) == 0) {
            ...
            if (value instanceof String[]) {
               ...
               System.out.println(
                     "nameMethode -----> " + m[i] + "----------arg:" + nameMethod + "-----------value-----:" + dd);
            }
   ...
   public Vector<Object[]> getClassAndtheMethodinPackagebyAnnotationname(String packageAbsolute, String pckgracine,
   ...
   public Vector<Object[]> getClassAndtheMethodinPackagebyAnnotationvalue(String packageAbsolute, String pckgracine,
         String annotationvalue, String nameMethod) throws Exception {
      Class[] classes = this.getAllClassinAllpackageBypackageRacine(packageAbsolute, pckgracine);
      Vector vCM = new Vector<Object[]>();
      Method[] methods = null;
      Object[] objCM = null;
